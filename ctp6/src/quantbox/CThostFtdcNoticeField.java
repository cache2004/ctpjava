package quantbox;
import com.sun.jna.Structure;
import java.util.Arrays;
import java.util.List;
/**
 * 客户通知<br>
 * <i>native declaration : ThostFtdcUserApiStruct.h:4081</i><br>
 * This file was autogenerated by <a href="http://jnaerator.googlecode.com/">JNAerator</a>,<br>
 * a tool written by <a href="http://ochafik.com/">Olivier Chafik</a> that <a href="http://code.google.com/p/jnaerator/wiki/CreditsAndLicense">uses a few opensource projects.</a>.<br>
 * For help, please visit <a href="http://nativelibs4java.googlecode.com/">NativeLibs4Java</a> , <a href="http://rococoa.dev.java.net/">Rococoa</a>, or <a href="http://jna.dev.java.net/">JNA</a>.
 */
public class CThostFtdcNoticeField extends Structure {
	/**
	 * 经纪公司代码<br>
	 * C type : TThostFtdcBrokerIDType
	 */
	public byte[] BrokerID = new byte[11];
	/**
	 * 消息正文<br>
	 * C type : TThostFtdcContentType
	 */
	public byte[] Content = new byte[501];
	/**
	 * 经纪公司通知内容序列号<br>
	 * C type : TThostFtdcSequenceLabelType
	 */
	public byte[] SequenceLabel = new byte[2];
	public CThostFtdcNoticeField() {
		super();
	}
	protected List<? > getFieldOrder() {
		return Arrays.asList("BrokerID", "Content", "SequenceLabel");
	}
	/**
	 * @param BrokerID 经纪公司代码<br>
	 * C type : TThostFtdcBrokerIDType<br>
	 * @param Content 消息正文<br>
	 * C type : TThostFtdcContentType<br>
	 * @param SequenceLabel 经纪公司通知内容序列号<br>
	 * C type : TThostFtdcSequenceLabelType
	 */
	public CThostFtdcNoticeField(byte BrokerID[], byte Content[], byte SequenceLabel[]) {
		super();
		if ((BrokerID.length != this.BrokerID.length)) 
			throw new IllegalArgumentException("Wrong array size !");
		this.BrokerID = BrokerID;
		if ((Content.length != this.Content.length)) 
			throw new IllegalArgumentException("Wrong array size !");
		this.Content = Content;
		if ((SequenceLabel.length != this.SequenceLabel.length)) 
			throw new IllegalArgumentException("Wrong array size !");
		this.SequenceLabel = SequenceLabel;
	}
	public static class ByReference extends CThostFtdcNoticeField implements Structure.ByReference {
		
	};
	public static class ByValue extends CThostFtdcNoticeField implements Structure.ByValue {
		
	};
}
