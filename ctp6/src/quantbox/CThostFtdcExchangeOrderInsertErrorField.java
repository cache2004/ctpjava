package quantbox;
import com.sun.jna.Structure;
import java.util.Arrays;
import java.util.List;
/**
 * 交易所报单插入失败<br>
 * <i>native declaration : ThostFtdcUserApiStruct.h:1471</i><br>
 * This file was autogenerated by <a href="http://jnaerator.googlecode.com/">JNAerator</a>,<br>
 * a tool written by <a href="http://ochafik.com/">Olivier Chafik</a> that <a href="http://code.google.com/p/jnaerator/wiki/CreditsAndLicense">uses a few opensource projects.</a>.<br>
 * For help, please visit <a href="http://nativelibs4java.googlecode.com/">NativeLibs4Java</a> , <a href="http://rococoa.dev.java.net/">Rococoa</a>, or <a href="http://jna.dev.java.net/">JNA</a>.
 */
public class CThostFtdcExchangeOrderInsertErrorField extends Structure {
	/**
	 * 交易所代码<br>
	 * C type : TThostFtdcExchangeIDType
	 */
	public byte[] ExchangeID = new byte[9];
	/**
	 * 会员代码<br>
	 * C type : TThostFtdcParticipantIDType
	 */
	public byte[] ParticipantID = new byte[11];
	/**
	 * 交易所交易员代码<br>
	 * C type : TThostFtdcTraderIDType
	 */
	public byte[] TraderID = new byte[21];
	/**
	 * 安装编号<br>
	 * C type : TThostFtdcInstallIDType
	 */
	public int InstallID;
	/**
	 * 本地报单编号<br>
	 * C type : TThostFtdcOrderLocalIDType
	 */
	public byte[] OrderLocalID = new byte[13];
	/**
	 * 错误代码<br>
	 * C type : TThostFtdcErrorIDType
	 */
	public int ErrorID;
	/**
	 * 错误信息<br>
	 * C type : TThostFtdcErrorMsgType
	 */
	public byte[] ErrorMsg = new byte[81];
	public CThostFtdcExchangeOrderInsertErrorField() {
		super();
	}
	protected List<? > getFieldOrder() {
		return Arrays.asList("ExchangeID", "ParticipantID", "TraderID", "InstallID", "OrderLocalID", "ErrorID", "ErrorMsg");
	}
	/**
	 * @param ExchangeID 交易所代码<br>
	 * C type : TThostFtdcExchangeIDType<br>
	 * @param ParticipantID 会员代码<br>
	 * C type : TThostFtdcParticipantIDType<br>
	 * @param TraderID 交易所交易员代码<br>
	 * C type : TThostFtdcTraderIDType<br>
	 * @param InstallID 安装编号<br>
	 * C type : TThostFtdcInstallIDType<br>
	 * @param OrderLocalID 本地报单编号<br>
	 * C type : TThostFtdcOrderLocalIDType<br>
	 * @param ErrorID 错误代码<br>
	 * C type : TThostFtdcErrorIDType<br>
	 * @param ErrorMsg 错误信息<br>
	 * C type : TThostFtdcErrorMsgType
	 */
	public CThostFtdcExchangeOrderInsertErrorField(byte ExchangeID[], byte ParticipantID[], byte TraderID[], int InstallID, byte OrderLocalID[], int ErrorID, byte ErrorMsg[]) {
		super();
		if ((ExchangeID.length != this.ExchangeID.length)) 
			throw new IllegalArgumentException("Wrong array size !");
		this.ExchangeID = ExchangeID;
		if ((ParticipantID.length != this.ParticipantID.length)) 
			throw new IllegalArgumentException("Wrong array size !");
		this.ParticipantID = ParticipantID;
		if ((TraderID.length != this.TraderID.length)) 
			throw new IllegalArgumentException("Wrong array size !");
		this.TraderID = TraderID;
		this.InstallID = InstallID;
		if ((OrderLocalID.length != this.OrderLocalID.length)) 
			throw new IllegalArgumentException("Wrong array size !");
		this.OrderLocalID = OrderLocalID;
		this.ErrorID = ErrorID;
		if ((ErrorMsg.length != this.ErrorMsg.length)) 
			throw new IllegalArgumentException("Wrong array size !");
		this.ErrorMsg = ErrorMsg;
	}
	public static class ByReference extends CThostFtdcExchangeOrderInsertErrorField implements Structure.ByReference {
		
	};
	public static class ByValue extends CThostFtdcExchangeOrderInsertErrorField implements Structure.ByValue {
		
	};
}
